...
---
⚙️ Execution Flow of n8n

The workflow is designed to run daily at 9:00 AM IST and automatically fetch, summarize, and publish drone-related news on LinkedIn. Below is the step-by-step execution:

⏰ Schedule Trigger

Starts the workflow every day at 9:00 AM IST.

📰 RSS Feed Readers

RSS Drone News → Fetches latest global drone-related articles.

RSS UAV Technology → Fetches updates on UAV research and innovation.

RSS DGCA Drones India → Fetches regulatory and industry news from India.

⏳ Limit Nodes

Each RSS feed is restricted to top 3 latest articles to keep content relevant and concise.

🔗 Merge Node

Combines all three feeds into a single stream of articles.

🔄 Loop Over Items

Iterates through each article one by one for processing.

🤖 Gemini (Message a Model)

Sends article content to Google Gemini.

Gemini generates:

3–5 paragraph professional summary
...
Formats Gemini’s response into a LinkedIn-ready post with:
...
Publishes the formatted post directly to LinkedIn.

Posts are made public for maximum reach.
